...
(async () => {
  ...
  try {
    ...
    // Mission — TJM libre vs marge sur SCR : marge appliquée directement au SCR (TJM = SCR × (1 + marge)).
    const mtjm = await p.evaluate(() => {
      var c = { scr: 500, contract: 'cdi' };
      var vide = { scr: 0, contract: 'cdi' };
      return {
        // 20 % sur SCR 500 : 500 × 1,20 = 600
        tjm20: tjmFromScr(c, 20),
        // 25 % sur SCR 500 : 500 × 1,25 = 625
        tjm25: tjmFromScr(c, 25),
        // 0 % : TJM = SCR
        tjm0: tjmFromScr(c, 0),
        // SCR absent : pas de calcul possible
        tjmVide: tjmFromScr(vide, 20)
      };
    });
    check('Mission TJM : marge appliquée au SCR (500 + 20 % = 600, + 25 % = 625, + 0 % = 500)',
